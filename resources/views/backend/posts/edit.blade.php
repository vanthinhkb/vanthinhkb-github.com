				...
				<div class="col-sm-9">
		            <div class="nav-tabs-custom">
		                <ul class="nav nav-tabs">
							...
							<li class="">
		                        <a href="#activity2" data-toggle="tab" aria-expanded="true">Đính kèm sản phẩm</a>
							</li>
							<li class="">
		                        ...
		                        <div class="row">
									<div class="col-sm-12">
										<?php 
											...
											$product = App\Models\Products::where('status', 1)->get();
										?>
										@foreach ($product as $item)
											<div class="col-sm-4">
												<input type="checkbox" name="idProduct[]" value="{{ $item->id }}" {{ !empty($id_product) ? in_array($item->id, $id_product) ? 'checked' : null : '' }}>
												<img src="{{ $item->image }}" alt="{{ $item->name }}" width="200px" height="100px" style="padding-left: 10px">
												<p>{{ $item->name }}</p>
											</div>
										@endforeach
									</div>
