    ...
    foreach ($data as $value) {
        ...
        if ($value->parent_id == $parent_id) {
            if ($str == '') {
                ...
                $strName_en = '<b>' . $str . $name_en . '</b>';
            } else {

                $strName = $str . $name;
                $strName_en = $str . $name_en;
            }
            echo '<tr class="odd">';
            echo '<td><input type="checkbox" name="chkItem[]" value="' . $id . '"></td>';
            echo '<td>
                        <a class="text-default" href="' . route('category.edit', $id) . '" title="Sửa">' . 'Tiếng việt: ' . $strName . '</a></br>
                        <a class="text-default" href="' . route('category.edit', $id) . '" title="Sửa">' . 'Tiếng Anh: ' . $strName_en . '</a></br>
...
function listCatePost($data, $parent_id = 0, $str = '')
{
    foreach ($data as $value) {
        ...
        if ($value->parent_id == $parent_id) {
            ...
            echo '<td><input type="checkbox" name="chkItem[]" value="' . $id . '"></td>';
            echo '<td>
                        <a class="text-default" href="' . route('categories-recruitment.edit', $id) . '" title="Sửa">' . 'Tiếng việt: ' . $strName . '</a></br>
                        <a class="text-default" href="' . route('categories-recruitment.edit', $id) . '" title="Sửa">' . 'Tiếng Anh: ' . $strName_en . '</a></br>
                        <a href="' . asset('danh-muc-tuyen-dung/' . $value->slug) . '" target="_blank"> <i class="fa fa-hand-o-right" aria-hidden="true"></i> Link: ' . asset('danh-muc-tuyen-dung/' . $value->slug) . ' </a>
                    </td>';
            echo ' <td><a href="' . route('categories-recruitment.edit', $id) . '" title="Sửa"> <i class="fa fa-pencil fa-fw"></i> Sửa</a> &nbsp; &nbsp; &nbsp;
                                <a href="javascript:;" class="btn-destroy" data-href="' . route('categories-recruitment.destroy', $id) . '" data-toggle="modal" data-target="#confim">
                                    <i class="fa fa-trash-o fa-fw"></i> Xóa
                                </a>
                            </td>';
            echo '</tr>';
